...
#should probably change this to Mark's copy of SGE.pm and Data.pm
######
...
if ($#ARGV !=1)  {
  print "usage: Polstats.pl month year \n";
  exit 0;
}
...
while  (<$fh>) {
      ...
      if ($start_time >= $T1 and $end_time < $T2) {
            ...
            my $mem_factor= 0;
            $mem_factor = ceil(SASGEtoBytes(SASGEacctResource($i, "h_vmem")) / SASGEtoBytes(Polhost2mem($hostname)));
...
##########################uncomment these lines for departmental usage. need file with dept names############
#open (my $fhand,"<","testnames.txt") or die ("can't open the file"); 
#
#my %owner_dept;
#while(my $line = <$fhand>){ 
