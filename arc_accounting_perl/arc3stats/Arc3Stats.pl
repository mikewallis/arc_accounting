#! /usr/bin/perl
use strict;
use lib '/home/home02/issmcal/Acc/lib';
use Time::Local;
use POSIX;
use SA::SGE;
use SA::Data::arc3;
...
if ($#ARGV !=1)  {
  print "usage: Arc3Stats.pl month year \n";
  exit 0;
}
...
while  (<$fh>) {
      ...
      if ($start_time >= $T1 and $end_time < $T2) {
            ...
            my $mem_factor= 0;
            $mem_factor = ceil(SASGEtoBytes(SASGEacctResource($i, "h_vmem")) / SASGEtoBytes(Arc3host2mem($hostname)));
